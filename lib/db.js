...
async function seedPostgres(force = false) {
  ...
  if (force) {
    // RESTART IDENTITY also resets the SERIAL sequences, so the fresh seed
    // starts again at ids 1–15 (keeps old product links valid after a reset)
    await pgPool.query('TRUNCATE order_items, orders, products, categories, admins, newsletter, sessions RESTART IDENTITY CASCADE');
  }
