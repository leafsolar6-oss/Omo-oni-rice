...
function seedOrders() {
  ...
  const insertOrder = db.prepare(`INSERT INTO orders
    (ref, customer_name, phone, email, address, city, state, delivery_method, delivery_fee, payment_method, subtotal, total, status, created_at)
    VALUES (@ref, @customer_name, @phone, @email, @address, @city, @state, @delivery_method, @delivery_fee, @payment_method, @subtotal, @total, @status, @created_at)`);
  ...
  statusPlan.forEach((status, i) => {
    ...
    const orderId = insertOrder.run({
      ...
      payment_method: payment, subtotal, total: subtotal + fee, status,
...
export function adminStats() {
  ...
  return { revenue, ordersCount, pending, productsCount, customers, days, recent };
}
